...
namespace hdcom
{
	...
	uint8_t* HdCommunication::GetTif_unknowndim_8bit(std::string path, int outshape[3], bool dspprogress, bool rescale)
	{
        if (path == "none")
        {
        ...
        if(hasEnding(path, "tif") || hasEnding(path, "tiff"))
        {
            uint8_t* output = Get2DTifImage_8bitPointer(path, outshape, rescale);
        ...
        else
        {
            ...
            if (rescale)
            {
                float* tmpimg = Get3DTifSequence_32bitPointer(filelist, outshape, dspprogress);
                long long int nslice = outshape[0]*outshape[1];
                long long int nstack = outshape[2]*nslice;
                float maxval = tmpimg[0]; float minval = tmpimg[0];

                #pragma omp parallel for
                for (long long int idx = 0; idx < nstack; idx++){
                    float val = tmpimg[idx];
                    if(val > maxval) maxval=val;
                    if(val < minval) minval = val;
                }

                uint8_t* img = (uint8_t*) malloc(nstack*sizeof(*img));

                #pragma omp parallel for
                for (long long int idx = 0; idx < nstack; idx++)
                    img[idx] = (uint8_t) ((tmpimg[idx]-minval)/(maxval-minval)*255);
    ...
    uint8_t* HdCommunication::Get2DTifImage_8bitPointer(std::string file, int outshape[2], bool rescale)
    {
        if(rescale)
        {
            //read into a temporary float and scale to 0 to 255 range
            float* tmpimg = Get2DTifImage_32bitPointer(file, outshape);
            long long int nslice = outshape[0]*outshape[1];
            float maxval = tmpimg[0]; float minval = tmpimg[0];
            for (long long int idx = 0; idx < nslice; idx++){float val = tmpimg[idx]; if(val > maxval) maxval=val; if(val < minval) minval = val;}
            uint8_t* img = (uint8_t*) malloc(nslice*sizeof(*img));
            for (long long int idx = 0; idx < nslice; idx++) img[idx] = (uint8_t) ((tmpimg[idx]-minval)/(maxval-minval)*255);
            free(tmpimg);
            return img;
        }
        ...
        if (tif)
        {
            ...
            image = (uint8_t*) malloc(height*width*sizeof(*image));
            int64_t pos = 0;
            ...
            for (int h = 0; h < height; h++)
            {
                ...
                else if (bps == 32)
                {
                    ...
                    for (int w = 0; w < width; w++)
                    {
                        tmpval1 = (unsigned char) data32[w];
                        image[pos] = tmpval1;
                        pos++;
                    }
                }
            ...
            if(bps == 8 || bps == 16) _TIFFfree(data16);
            else if (bps == 32) _TIFFfree(data32);
			#pragma GCC diagnostic warning "-Wmaybe-uninitialized"
        }
        else
        {
        	image = (uint8_t*) malloc(0*sizeof(*image));
    ...
    template <typename T> void HdCommunication::Save2DTifImage_8bit(T* image, int imgshape[2], std::string path, std::string name, long long int firstpos, bool rescale){
        int64_t pos = 0;
        ...
        //********************************************
        tdata_t buf = _TIFFmalloc(TIFFScanlineSize(output_image));

        if(typeid(T) != typeid(uint8_t) || rescale)
        {
            long long int nslice = height*width;
            uint8_t* new_image = (uint8_t*) calloc(nslice, sizeof(*new_image));
            ...
            if(rescale)
            {
                ...
                for (long long int idx = 1; idx < nslice; idx++)
                {
                    T val = image[idx];
                    if (val > maxval) maxval = val;
                    if (val < minval) minval = val;
                ...
                for (long long int idx = 0; idx < nslice; idx++)
                    new_image[idx] = (uint8_t) ((image[idx]-minval)/(maxval-minval)*255);
            }
            else
            {
                for (long long int idx = 0; idx < nslice; idx++)
                    new_image[idx] = (uint8_t) image[idx];
            }

            for (int h = 0; h < height; h++)
            {
                buf = &new_image[pos+firstpos];
                pos += width;
    ...
    template void HdCommunication::Save2DTifImage_8bit<uint8_t>(uint8_t* image, int imgshape[2], std::string path, std::string name, long long int firstpos, bool rescale);
    template void HdCommunication::Save2DTifImage_8bit<int>(int* image, int imgshape[2], std::string path, std::string name, long long int firstpos, bool rescale);
    template void HdCommunication::Save2DTifImage_8bit<float>(float* image, int imgshape[2], std::string path, std::string name, long long int firstpos, bool rescale);

    template <typename T> void HdCommunication::Save2DTifImage_32bit(T* image, int imgshape[2], std::string path, std::string name, long long int firstpos){
        long long int pos = firstpos;
        ...
        {
            float* line = (float*) malloc(width*sizeof(*line));
    ...
    template void HdCommunication::Save2DTifImage_32bit<uint8_t>(uint8_t* image, int imgshape[2], std::string path, std::string name, long long int firstpos);
    template void HdCommunication::Save2DTifImage_32bit<int>(int* image, int imgshape[2], std::string path, std::string name, long long int firstpos);
    template void HdCommunication::Save2DTifImage_32bit<float>(float* image, int imgshape[2], std::string path, std::string name, long long int firstpos);
    ...
    template <typename T> void HdCommunication::SaveTifSequence_8bit(T* image, int imgshape[3], std::string path, std::string name, bool dspprogress, bool rescale){
        const int zfill = 4;
        long long int nslice = imgshape[0]*imgshape[1];

        T minval = 0; T maxval = 1;
        if (rescale)
        {
            minval = image[0]; maxval = image[0];
            #pragma omp parallel for reduction(min: minval), reduction(max: maxval)
            for (long long int idx = 1; idx < imgshape[2]*nslice; idx++)
            {
                T val = image[idx];
                if(val > maxval) maxval = val;
                if(val < minval) minval = val;
            ...
            for(int i=0; i<imgshape[2]; i++)
            {
                ...
                uint8_t* slice = (uint8_t*) calloc(nslice, sizeof(*slice));
                ...
                for (long long int idx = 0; idx < nslice; idx++)
                    slice[idx] = (uint8_t) ((image[idx]-minval)/(maxval-minval)*255);

                //Call 2D-Save at position in vector that is the beginning of the current slice
                Save2DTifImage_8bit(slice, imgshape, path, name+id);
    ...
    template void HdCommunication::SaveTifSequence_8bit<uint8_t>(uint8_t* image, int imgshape[3], std::string path, std::string name, bool dspprogress, bool rescale);
    template void HdCommunication::SaveTifSequence_8bit<int>(int* image, int imgshape[3], std::string path, std::string name, bool dspprogress, bool rescale);
    template void HdCommunication::SaveTifSequence_8bit<float>(float* image, int imgshape[3], std::string path, std::string name, bool dspprogress, bool rescale);

    template <typename T> void HdCommunication::SaveTifSequence_32bit(T *image, int imgshape[3], std::string path, std::string name, bool dspprogress){
		const int zfill = 4;
	...
	template void HdCommunication::SaveTifSequence_32bit<uint8_t>(uint8_t* image, int imgshape[3], std::string path, std::string name, bool dspprogress);
	template void HdCommunication::SaveTifSequence_32bit<int>(int* image, int imgshape[3], std::string path, std::string name, bool dspprogress);
	template void HdCommunication::SaveTifSequence_32bit<float>(float* image, int imgshape[3], std::string path, std::string name, bool dspprogress);
    ...
    void HdCommunication::save2inr(std::string outpath, std::string filename, uint8_t *image, int shape[3])
    {
        long long int nslice = shape[0]*shape[1];
        long long int nstack = shape[2]*nslice;

        std::string endianness = "decm";
        if (htonl(47) == 47) endianness = "sun";

        std::string header = "#INRIMAGE-4#{\nXDIM="+std::to_string(shape[0])+"\nYDIM="
                        +std::to_string(shape[1])+"\nZDIM="+std::to_string(shape[2])+
                        "\nVDIM=1\nTYPE=unsigned fixed\nPIXSIZE=8 bits\nSCALE=2**0\nCPU="+endianness+"\nVX=1\nVY=1\nVZ=1\n#GEOMETRY=CARTESIAN#\n";

        while (((header.length()+4)%256) != 0) header += "\n";
        header += "##}\n";

        std::ofstream output;
        output.open( outpath+"/"+filename+".inr", std::ios::out | std::ios::binary);

        output << header.c_str();

        output.write( (char*)&image[0], nstack*sizeof( uint8_t ));
        output.close();
    }
